...
class NAODataset(torch.utils.data.Dataset):
    def __init__(self, inputs, targets=None, train=True, sos_id=0, eos_id=0, swap=False, encoder_length: int = 20):
    ...
    def __getitem__(self, index):
        ...
        if self.swap:
            ...
            encoder_input = encoder_input[:4 * a] + encoder_input[4 * a + 2:4 * a + 4] + \
                            encoder_input[4 * a:4 * a + 2] + encoder_input[4 * (a + 1):self.encoder_length + 4 * b] + \
                            encoder_input[
                            self.encoder_length + 4 * b + 2:self.encoder_length + 4 * b + 4] + encoder_input[
                                                                                               self.encoder_length + 4 * b:self.encoder_length + 4 * b + 2] + \
                            encoder_input[self.encoder_length + 4 * (b + 1):]
        if self.train:
